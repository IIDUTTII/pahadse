...
// ✨ DECOUPLED: Only importing functions from our single source of truth (db.js)
...
const handleGoogleAuth = async () => {
  ...
  try {
    // Calling the function from db.js
...
const handleEmailAuth = async () => {
  ...
  try {
    // Calling the centralized function from db.js
    ...
    <div class="split-register-card fade-in">
      ...
      <div class="form-entry-pane">
        ...
        <div class="input-wrapper-field">
          ...
          <input v-model="fullName" type="text" placeholder="e.g. Rahul Sharma" :disabled="loading" />
        </div>
...
.input-wrapper-field { display: flex; flex-direction: column; gap: 6px; margin-bottom: 12px; }
...
.google-federation-btn { width: 100%; padding: 12px; background-color: #ffffff; color: #111827; border: 2px solid #111827; border-radius: 30px; font-size: 0.9rem; font-weight: 700; display: flex; align-items: center; justify-content: center; gap: 10px; cursor: pointer; margin-bottom: 12px; transition: all 0.2s; }
.google-federation-btn:hover:not(:disabled) { background-color: #f3f4f6; }
.email-toggle-btn { width: 100%; padding: 12px; background: #f1f5f9; border: none; border-radius: 30px; font-size: 0.9rem; font-weight: 700; color: #0f2a1f; cursor: pointer; margin-bottom: 8px; }
.email-toggle-btn:hover { background: #e2e8f0; }
.email-form-expansion { display: flex; flex-direction: column; gap: 8px; margin-top: 8px; }
.action-row { display: flex; gap: 12px; align-items: center; margin-top: 12px; }
.cancel-text-btn { background: transparent; border: none; color: #64748b; font-weight: 700; cursor: pointer; padding: 10px; }
.cancel-text-btn:hover { color: #dc2626; }
.submit-action-btn { width: 100%; padding: 12px; background-color: #111827; color: #faf6f0; border: none; border-radius: 30px; font-size: 0.9rem; font-weight: 700; cursor: pointer; transition: background 0.2s; }
.submit-action-btn:hover:not(:disabled) { background-color: #16a34a; }
.alert-banner { padding: 10px 14px; border-radius: 8px; font-size: 0.85rem; font-weight: 600; margin-bottom: 16px; display: flex; align-items: center; gap: 8px; }
...
.slide-down-enter-to, .slide-down-leave-from { opacity: 1; transform: translateY(0); max-height: 500px; }

@media (max-width: 840px) {
  .split-register-card { grid-template-columns: 1fr; max-width: 500px; }
  .brand-ambiance-pane { padding: 60px 30px; border-bottom: 2px solid #111827; background-position: 80% 80%; }
