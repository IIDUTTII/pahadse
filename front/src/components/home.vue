...
<template>
  <div class="page">
    ...
    <div v-else class="product-grid">
      <div v-for="product in products" :key="product.id" class="product-card" @click="goToProduct(product)">
...
<style scoped>
/* 🌿 GREEN THEME – MATCHES FOOTER (#0F2A1F, #2A5C3E) */
.page {
  --footer-green: #0F2A1F;
...
/* Hero Section – using footer green */
.hero {
  background: linear-gradient(135deg, var(--footer-green) 0%, var(--footer-green-light) 100%);
...
.skeleton-card {
  height: 380px;
  background: linear-gradient(90deg, #E9EFE3 25%, #F4F8ED 50%, #E9EFE3 75%);
...
.emoji-fallback {
  font-size: 64px;
  background: linear-gradient(145deg, #E6EDDC, #F9F6EA);
  width: 100%;
  height: 100%;
  display: flex;
  align-items: center;
