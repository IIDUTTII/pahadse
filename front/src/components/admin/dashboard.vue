...
import { fetchUserRole, updateProduct, deleteProduct, logoutUser, fetchAllUsersFromDb, updateUserRoleInDb, fetchCoupons, saveCoupons, deleteCouponFromDb, setCodStatus } from '../db.js'
...
const newCoupon       = ref({
  ...
  minOrderAmount: 0, minItems: 0, expiresAt: '', active: true
})
...
// ✨ FIXED: Now deletes permanently from DB directly
const removeCoupon = async (idx) => {
  ...
  if (confirm(`Permanently delete the ${targetCoupon.code} voucher?`)) {
    try {
      ...
      alert("Failed to delete coupon from database: " + e.message)
    }
  }
}

// ✨ FIXED: Saves toggled active/inactive states directly to DB instantly
  ...
  <div class="shell">
    ...
    <header class="topbar">
      <div class="topbar-left">
        <button class="hamburger" @click="sidebarOpen = !sidebarOpen"><span></span><span></span><span></span></button>
        <div class="brand">🏔️ {{ userRole === 'superadmin' ? 'SuperAdmin' : 'Admin' }}</div>
      </div>
      <input v-show="activeTab === 'products'" v-model="searchText" class="search-input" placeholder="Search products…" />
      <div class="topbar-right">
        <button class="cod-btn" :class="isCodActive ? 'cod-on' : 'cod-off'" @click="toggleCod" :title="'COD ' + (isCodActive ? 'click to disable' : 'click to enable')">
          <span class="cod-dot"></span><span class="cod-label">COD: {{ isCodActive ? 'On' : 'Off' }}</span>
        </button>
    ...
    <div class="body">
      ...
      <main class="workspace">
        ...
        <div v-if="activeTab === 'products'" class="fade-in">
          <div class="ws-head">
            <div><h2 class="ws-title">Products Inventory</h2><p class="ws-sub">{{ filteredProducts.length }} items</p></div>
            <div class="ws-actions">
              <select v-model="selectedCategory" class="select-pill">
                <option value="All">All Categories</option><option>Spices & Herbs</option><option>Ghee & Oils</option><option>Juices</option><option>Sweets</option><option>Himalayan Teas</option>
              </select>
          ...
          <div class="product-cards mobile-only">
            <div v-for="p in filteredProducts" :key="p.id" class="p-card" :class="{ inactive: !p.isActive }">
              <div class="p-card-top">
                <div class="thumb-cell"><img v-if="p.imageUrls?.find(u => u?.trim())" :src="p.imageUrls.find(u => u?.trim())" class="thumb-img" /><span v-else class="thumb-emoji">{{ p.emoji }}</span></div>
                <div class="p-card-info">
                  <div class="item-name">{{ p.name }}</div>
                  <div class="item-sub">{{ p.weight }} · <span class="cat-pill">{{ p.category }}</span></div>
                  <div class="price-disc-row"><span class="price-val">₹{{ p.price }}</span><span v-if="p.discount?.isDiscounted" class="disc-tag">{{ p.discount.percent }}% off</span></div>
                </div>
              </div>
              <div class="p-card-actions">
                ...
                <button class="btn-edit" @click="goEdit(p)">Edit</button><button v-if="userRole === 'superadmin'" class="btn-del" @click="handleDel(p.id)">Del</button>
              </div>
            </div>
          </div>
              ...
              <tbody>
                <tr v-for="p in filteredProducts" :key="p.id" :class="{ 'row-inactive': !p.isActive }">
                  <td>
                    <div class="item-meta">
                      <div class="thumb-cell"><img v-if="p.imageUrls?.find(u => u?.trim())" :src="p.imageUrls.find(u => u?.trim())" class="thumb-img" /><span v-else class="thumb-emoji">{{ p.emoji }}</span></div>
                      <div><div class="item-name">{{ p.name }}</div><div class="item-sub">{{ p.weight }}</div></div>
                    </div>
                  </td>
                  <td><span class="cat-pill">{{ p.category }}</span></td>
                  <td><div class="price-disc-col"><span class="price-val">₹{{ p.price }}</span><span v-if="p.discount?.isDiscounted" class="disc-tag">{{ p.discount.percent }}% off</span></div></td>
                  ...
                  <td><div class="row-actions"><button class="btn-edit" @click="goEdit(p)">Edit</button><button v-if="userRole === 'superadmin'" class="btn-del" @click="handleDel(p.id)">Delete</button></div></td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div v-if="activeTab === 'orders'" class="fade-in">
          <div class="ws-head"><div><h2 class="ws-title">Orders</h2><p class="ws-sub">{{ ordersList.length }} total orders</p></div></div>
          <div class="order-cards mobile-only">
            <div v-for="o in ordersList" :key="o.id" class="o-card">
              <div class="o-card-row"><code class="id-code">{{ o.id?.substring(0, 10) }}…</code><span :class="['status-pill', statusClass(o.shippingStatus)]">{{ o.shippingStatus || 'pending' }}</span></div>
              <div class="o-card-name">{{ o.customerName || '—' }}</div><div class="o-card-sub">{{ fmtDate(o.createdAt) }} · ₹{{ o.amount }}</div>
              <button class="btn-view" @click="openOrder(o)">View Details →</button>
            </div>
          </div>
          <div class="table-wrap desktop-only">
            <table class="data-table">
              <thead><tr><th>Order ID / Date</th><th>Customer</th><th>Address</th><th>Total</th><th>Status</th><th>Action</th></tr></thead>
              <tbody>
                <tr v-for="o in ordersList" :key="o.id">
                  <td><code class="id-code">{{ o.id?.substring(0, 10) }}…</code><div class="item-sub">{{ fmtDate(o.createdAt) }}</div></td>
                  <td><div class="item-name">{{ o.customerName || '—' }}</div><div class="item-sub">📞 {{ o.phone }}</div></td>
                  <td class="addr-cell">{{ o.address }}</td><td class="price-val">₹{{ o.amount }}</td>
        ...
        <div v-if="activeTab === 'support'" class="fade-in support-layout">
          <div class="ws-head"><div><h2 class="ws-title">Support Desk</h2><p class="ws-sub">Customer queries</p></div></div>
          <div class="chat-shell">
            <div class="thread-list">
              <div class="thread-list-head">Conversations</div>
              <div v-if="supportThreads.length === 0" class="thread-empty">No chats yet</div>
              <div v-for="t in supportThreads" :key="t.userId" :class="['thread-item', { selected: selectedChatUserId === t.userId }]" @click="selectedChatUserId = t.userId">
                <div class="thread-name">{{ t.userName || 'Anonymous' }}</div><div class="thread-uid">{{ t.userId.substring(0, 8) }}…</div>
              </div>
            </div>
            <div class="chat-window">
              <div v-if="!selectedChatUserId" class="chat-placeholder"><span>💬</span><p>Select a conversation</p></div>
              <template v-else>
                <div class="chat-messages">
                  <div v-for="(msg, i) in activeChatDoc?.messages || []" :key="i" :class="['bubble-wrap', msg.role === 'admin' ? 'mine' : 'theirs']">
        ...
        <div v-if="activeTab === 'users'" class="fade-in">
          <div class="ws-head"><div><h2 class="ws-title">Users</h2><p class="ws-sub">{{ usersList.length }} registered</p></div></div>
          <div class="user-cards mobile-only">
            <div v-for="u in usersList" :key="u.id" class="u-card">
              <div class="u-name">{{ u.displayName || 'User' }}</div><div class="u-email">{{ u.email }}</div>
              <div class="u-row">
                <span :class="['role-pill', u.role === 'superadmin' ? 'role-super' : u.role === 'admin' ? 'role-admin' : 'role-user']">{{ u.role || 'user' }}</span>
                <div v-if="userRole === 'superadmin'" class="role-btns"><button class="btn-role" @click="handleRole(u.id, 'user')">User</button><button class="btn-role" @click="handleRole(u.id, 'admin')">Admin</button><button class="btn-role" @click="handleRole(u.id, 'superadmin')">Super</button></div>
              </div>
            </div>
          </div>
          <div class="table-wrap desktop-only">
            <table class="data-table">
              <thead><tr><th>Name / UID</th><th>Email</th><th>Role</th><th>Change Role</th></tr></thead>
              <tbody>
                <tr v-for="u in usersList" :key="u.id">
                  <td><div class="item-name">{{ u.displayName || 'User' }}</div><code class="id-code">{{ u.id?.substring(0, 8) }}…</code></td>
                  <td class="item-sub">{{ u.email }}</td>
                  <td><span :class="['role-pill', u.role === 'superadmin' ? 'role-super' : u.role === 'admin' ? 'role-admin' : 'role-user']">{{ u.role || 'user' }}</span></td>
                  <td>
                    <div v-if="userRole === 'superadmin'" class="row-actions"><button class="btn-role" @click="handleRole(u.id, 'user')">User</button><button class="btn-role" @click="handleRole(u.id, 'admin')">Admin</button><button class="btn-role" @click="handleRole(u.id, 'superadmin')">Super</button></div>
    ...
    <transition name="slide-up">
      <div v-if="showCouponPanel" class="coupon-overlay" @click.self="showCouponPanel = false">
        <div class="coupon-panel">
          <div class="panel-head"><h3>Coupon Management</h3><button class="x-btn" @click="showCouponPanel = false">✕</button></div>

          <div class="coupon-form">
            <h4 class="form-section-label">Add New Coupon</h4>
            <div class="coupon-grid">
              <div class="cf-field"><label>Code</label><input v-model="newCoupon.code" class="cf-input" placeholder="PAHAD10" style="text-transform:uppercase" /></div>
              <div class="cf-field"><label>Type</label><select v-model="newCoupon.type" class="cf-input"><option value="percent">Percent (%)</option><option value="flat">Flat Amount (₹)</option></select></div>
              <div class="cf-field"><label>Discount {{ newCoupon.type === 'percent' ? '(%)' : '(₹)' }}</label><input v-model.number="newCoupon.discount" type="number" class="cf-input" min="1" /></div>
              <div class="cf-field"><label>Min Order Amount (₹)</label><input v-model.number="newCoupon.minOrderAmount" type="number" class="cf-input" min="0" /></div>
              <div class="cf-field"><label>Min Items in Cart</label><input v-model.number="newCoupon.minItems" type="number" class="cf-input" min="0" /></div>
              
              <!-- ✨ FIXED: Expiration defaults to HTML native calendar date-picker -->
              <div class="cf-field">
                <label>Expiration Date</label>
                <input v-model="newCoupon.expiresAt" type="date" class="cf-input" />
              </div>
            </div>
            <div class="coupon-form-foot">
              <label class="toggle-label"><input type="checkbox" v-model="newCoupon.active" /> Active</label>
          ...
          <div class="panel-foot"><button class="btn-primary" :disabled="couponSaving" @click="saveCouponList">{{ couponSaving ? 'Saving…' : '💾 Save All Coupons' }}</button></div>
        </div>
      </div>
    </transition>
    ...
    <transition name="fade">
      <div v-if="showOrderModal && selectedOrder" class="modal-backdrop" @click.self="closeOrder">
        <div class="modal-box">
          <header class="modal-head"><div><h3 class="modal-title">Order Details</h3><code class="modal-id">{{ selectedOrder.id }}</code></div><button class="x-btn" @click="closeOrder">✕</button></header>
          <div class="modal-body">
            <div class="modal-status-bar"><span :class="['status-pill lg', statusClass(selectedOrder.shippingStatus)]">{{ selectedOrder.shippingStatus || 'pending' }}</span><span class="modal-date">{{ fmtDate(selectedOrder.createdAt) }}</span></div>
            <section class="modal-section">
              <div class="modal-section-title">Customer Information</div>
              <div class="info-grid">
                <div class="info-cell"><span class="info-label">Name</span><strong class="info-val">{{ selectedOrder.customerName }}</strong></div>
                <div class="info-cell"><span class="info-label">Phone</span><strong class="info-val">{{ selectedOrder.phone }}</strong></div>
                <div class="info-cell full"><span class="info-label">Delivery Address</span><div class="addr-block">{{ selectedOrder.address }}</div></div>
              </div>
            </section>
            <section class="modal-section">
              <div class="modal-section-title">Items Ordered</div>
              <div class="items-list">
                <div v-for="item in selectedOrder.items" :key="item.productId" class="item-strip"><span class="item-emoji">{{ item.emoji || '📦' }}</span><span class="item-strip-name">{{ item.name }}</span><span class="item-strip-qty">× {{ item.quantity }}</span><strong class="item-strip-price">₹{{ item.price * item.quantity }}</strong></div>
              </div>
            </section>
            <section class="modal-section">
              ...
              <div class="pricing-block">
                ...
                <!-- ✨ NEW: Render Coupon Math to the Admin -->
                <div v-if="selectedOrder.couponCode" class="price-row" style="color: #15803d;">
            ...
            <section v-if="selectedOrder.shippingStatus !== 'shipped' && selectedOrder.shippingStatus !== 'delivered' && selectedOrder.shippingStatus !== 'rejected'" class="modal-section">
              <div class="modal-section-title">Order Actions</div>
              <div v-if="selectedOrder.shippingStatus === 'pending'" style="margin-bottom:14px"><button class="btn-confirm-full" @click="confirmOrder(selectedOrder.id)">✓ Confirm Order</button></div>
              <div class="action-block"><label class="action-label">Shiprocket AWB Tracking ID</label><div class="tracking-row"><input :id="'awb_' + selectedOrder.id" type="text" :value="selectedOrder.trackingId !== 'PENDING_DISPATCH' ? selectedOrder.trackingId : ''" placeholder="Paste AWB tracking number…" class="cf-input" /><button class="btn-primary" @click="saveTracking(selectedOrder.id, 'awb_' + selectedOrder.id)">Mark Shipped</button></div></div>
              <div class="action-block reject-zone"><label class="action-label danger">Cancel & Reject Order</label><textarea v-model="rejectionComment" class="reject-textarea" placeholder="Provide reason for customer (e.g., Refund will be processed in 3-5 days via Razorpay)…"></textarea><button class="btn-reject-full" @click="rejectOrder">✕ Reject Order</button></div>
            </section>

            <div v-if="selectedOrder.shippingStatus === 'shipped' || selectedOrder.shippingStatus === 'delivered'" class="shipped-tag">🚚 Tracking ID: <strong>{{ selectedOrder.trackingId }}</strong></div>
            <div v-if="selectedOrder.shippingStatus === 'rejected'" class="rejected-tag"><strong>⚠️ Order Rejected</strong><p v-if="selectedOrder.adminComment">Reason: "{{ selectedOrder.adminComment }}"</p></div>
          </div>
        </div>
      </div>
    </transition>
...
.shell { --bg: #F0F2F5; --white: #FFFFFF; --border: #E2E8F0; --text: #0F172A; --muted: #64748B; --green: #15803D; --red: #DC2626; --amber: #92400E; --blue: #2563EB; --radius: 10px; display: flex; flex-direction: column; min-height: 40vh; background: var(--bg); font-family: 'DM Sans', system-ui, sans-serif; color: var(--text); box-sizing: border-box; padding-top: 150px; }
.topbar { position: fixed; top: 90px; left: 0; right: 0; z-index: 200; height: 60px; background: var(--white); border-bottom: 1px solid var(--border); display: flex; align-items: center; gap: 14px; padding: 0 20px; box-shadow: 0 1px 4px rgba(0,0,0,.06); }
.topbar-left  { display: flex; align-items: center; gap: 10px; }
.topbar-right { display: flex; align-items: center; gap: 8px; margin-left: auto; }
.brand { font-weight: 800; font-size: 1rem; color: var(--green); white-space: nowrap; }
.hamburger { display: none; flex-direction: column; gap: 4px; width: 32px; height: 32px; border: none; background: transparent; cursor: pointer; padding: 6px; border-radius: 6px; }
.hamburger span { display: block; width: 100%; height: 2px; background: var(--muted); border-radius: 2px; }
@media (max-width: 768px) { .hamburger { display: flex; } }
.search-input { flex: 1; max-width: 300px; padding: 8px 14px; border: 1.5px solid var(--border); border-radius: 20px; font-size: .9rem; outline: none; background: var(--bg); color: var(--text); font-weight: 600; }
.search-input:focus { border-color: var(--green); background: var(--white); }
.cod-btn { display: flex; align-items: center; gap: 6px; padding: 6px 14px; border-radius: 20px; border: 2px solid; font-size: .8rem; font-weight: 800; cursor: pointer; white-space: nowrap; }
...
.top-btn { padding: 6px 14px; border: 1.5px solid var(--border); border-radius: 20px; background: var(--white); font-size: .82rem; font-weight: 700; cursor: pointer; color: var(--text); display: flex; align-items: center; gap: 4px; white-space: nowrap; }
.top-btn.danger { border-color: #fca5a5; color: var(--red); }
.top-btn:hover  { background: var(--bg); }
.body { display: flex; flex: 1; }
.sidebar { width: 200px; background: var(--white); border-right: 1px solid var(--border); padding: 16px 10px; display: flex; flex-direction: column; gap: 4px; flex-shrink: 0; }
...
@media (max-width: 768px) { .sidebar { position: fixed; top: 60px; left: -220px; bottom: 0; z-index: 150; width: 220px; transition: left .25s ease; box-shadow: 4px 0 16px rgba(0,0,0,.1); } .sidebar.sidebar-open { left: 0; } }
...
@media (max-width: 768px) { .mobile-only  { display: flex; flex-direction: column; gap: 12px; } .desktop-only { display: none; } }
...
@media (max-width: 768px) { .thread-list { width: 120px; } .thread-name { font-size: .8rem; } }
